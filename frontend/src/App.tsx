...
import { 
  ...
  AlertCircle, ChevronRight, ChevronLeft, LayoutDashboard, Search,
  RefreshCw, FileText, Calendar
} from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { STOCKS_DATA } from './data';
import { StockData, ReportData } from './types';
...
function AppContent() {
  ...
  // Fetch dates when ticker changes
  useEffect(() => {
    if (selectedTicker) {
      fetch(`/api/reports/${selectedTicker}/dates`)
        ...
        .then(data => {
          if (Array.isArray(data)) {
            ...
            // Default to today if available, otherwise Live
  ...
  // Fetch report when ticker or date changes
  useEffect(() => {
    if (selectedTicker) {
      const dateKey = selectedDate.replace(/-/g, '');
      setIsReportLoading(true);
      fetch(`/api/reports/${selectedTicker}/${dateKey}`)
        .then(res => {
  ...
  const chartData = useMemo(() => {
    if (!selectedStock || !Array.isArray(selectedStock.quarterly_revenue)) return [];
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-100 font-sans overflow-hidden">
      ...
      <aside className="w-72 bg-zinc-900 border-r border-zinc-800 flex flex-col">
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
          <div className="px-3 py-2 text-[10px] font-bold text-zinc-600 uppercase tracking-widest">View Mode</div>
          <div className="px-3 mb-6 space-y-3 relative">
            ...
            >
              ...
              <ChevronRight className={cn("w-3 h-3 text-zinc-600 transition-transform", isCalendarOpen && "rotate-90")} />
            </button>
                ...
                >
                  ...
                  <div className="grid grid-cols-7 gap-1 mb-4 min-h-[180px]">
                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map(day => (
                      <div key={day} className="text-[10px] font-bold text-zinc-600 text-center py-1">{day}</div>
                    ))}
                    {(() => {
                      ...
                      for (let d = 1; d <= daysInMonth; d++) {
                        ...
                        const dateKey = dateObj.toISOString().split('T')[0].replace(/-/g, '');
                        const hasReport = availableDates.includes(dateKey);
                        const isSelected = selectedDate === dateKey;
                        const isToday = new Date().toISOString().split('T')[0].replace(/-/g, '') === dateKey;
                        ...
                        days.push(
                          <button
                            ...
                            className={cn(
                              ...
                              hasReport ? "text-zinc-200 hover:bg-emerald-500/20 hover:text-emerald-400 cursor-pointer" : "text-zinc-700 cursor-not-allowed",
                              isSelected && "bg-emerald-500 text-zinc-950 font-bold hover:bg-emerald-500 hover:text-zinc-950",
                  ...
                  <button 
                    ...
                    className={cn(
                      ...
                      selectedDate === 'Live' ? "bg-emerald-500 text-zinc-950" : "bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200"
                    )}
                  >
                    Live Data (Current)
                  </button>
          ...
          <div className="px-3 py-2 text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Watchlist</div>
              ...
              <div className="flex flex-col items-end">
                ...
                <span className={cn(
                  ...
                  selectedTicker === stock.ticker 
                    ...
                    : (stock.grade && stock.grade.startsWith('A')) ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500"
                )}>
                  {stock.grade}
                </span>
              </div>
        ...
        {selectedStock ? (
          <div className="max-w-7xl mx-auto">
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <span className="text-zinc-400 text-sm flex items-center">
                    ...
                    {reportData ? reportData.analysis.sentiment + " (Report)" : selectedStock.signal}
                  </span>
                  <span className="text-zinc-700">•</span>
                  <span className="text-zinc-400 text-sm flex items-center">
                    <AlertCircle className="w-4 h-4 mr-1 text-amber-500" />
                    {reportData ? `Score: ${reportData.analysis.score}` : `Zone: ${selectedStock.zone}`}
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <MetricCard 
                title="Price" 
                value={!reportData ? formatCurrency(selectedStock.price) : formatCurrency(reportData.analysis.chart_data[reportData.analysis.chart_data.length - 1]?.value || 0)} 
                subValue={!reportData ? `52W Range: ${formatCurrency(selectedStock.low_52w)} - ${formatCurrency(selectedStock.high_52w)}` : 'Closing Price in Report'}
                icon={DollarSign}
                trend={!reportData 
                  ? (selectedStock.price > selectedStock.my_fair_mid ? 'down' : 'up')
                  : (reportData.analysis.sentiment === 'Bullish' ? 'up' : reportData.analysis.sentiment === 'Bearish' ? 'down' : 'neutral')
                }
              />
              <MetricCard 
                title="P/E Ratio" 
                value={!reportData 
                  ? (typeof selectedStock.ttm_pe === 'number' ? selectedStock.ttm_pe.toFixed(2) : 'N/A')
                  : (reportData.analysis.metrics.pe_ratio?.toFixed(2) || 'N/A')
                } 
                subValue={!reportData ? `Forward: ${typeof selectedStock.forward_pe === 'number' ? selectedStock.forward_pe.toFixed(2) : 'N/A'}` : 'Reported P/E'}
                icon={BarChart3}
              ...
              <MetricCard 
                ...
                value={typeof selectedStock.rev_growth_yoy === 'number' ? `${selectedStock.rev_growth_yoy.toFixed(1)}%` : 'N/A'} 
              ...
              <div className="lg:col-span-2 bg-zinc-900 p-8 rounded-2xl border border-zinc-800 shadow-xl">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="text-lg font-bold text-white">
                    {!reportData ? 'Financial Performance (Quarterly)' : 'Intraday Performance (Report)'}
                  </h3>
            ...
            <div className="space-y-8">
              <FairValueGauge 
                current={!reportData ? selectedStock.price : (reportData.analysis.chart_data?.[reportData.analysis.chart_data.length - 1]?.value || 0)}
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-mono font-bold text-emerald-400">
                        {typeof selectedStock.roe === 'number' ? selectedStock.roe.toFixed(2) : '-'}%
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-zinc-500 text-sm">FCF Yield</span>
                      <span className="font-mono font-bold text-white">
                        {typeof selectedStock.fcf_yield === 'number' ? selectedStock.fcf_yield.toFixed(2) : '-'}%
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                ...
                <div className="bg-zinc-900 text-white p-8 rounded-2xl border border-zinc-800 shadow-xl">
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      ...
                      <span className={cn(
                        "font-bold",
                        reportData?.analysis.sentiment === 'Bullish' ? "text-emerald-500" : 
                        reportData?.analysis.sentiment === 'Bearish' ? "text-rose-500" : "text-amber-500"
                      )}>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-mono font-bold text-white">{reportData?.analysis.score}/100</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-zinc-500 text-sm">Risk Level</span>
                      <span className={cn(
                        "font-bold",
                        reportData?.analysis.metrics.risk_level === 'Low' ? "text-emerald-500" : 
                        reportData?.analysis.metrics.risk_level === 'High' ? "text-rose-500" : "text-amber-500"
                      )}>
                        {reportData?.analysis.metrics.risk_level}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-zinc-500 text-sm">Growth Rate</span>
                      <span className="font-mono font-bold text-white">{reportData?.analysis.metrics.growth_rate}%</span>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <tbody className="divide-y divide-zinc-800">
                    {selectedStock.quarterly_revenue.map((q, idx) => (
                      <tr key={idx} className="hover:bg-zinc-800/50 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-sm font-mono text-zinc-400">
                          {typeof q.operatingIncome === 'number' ? formatCurrency(q.operatingIncome) : '-'}
                        </td>
                        <td className="px-6 py-4 text-sm font-mono text-zinc-400">
                          {typeof q.netIncome === 'number' ? formatCurrency(q.netIncome) : '-'}
                        </td>
                        <td className="px-6 py-4 text-sm font-mono text-zinc-400">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-mono font-bold text-white">
                              {typeof q.grossMargin === 'number' ? q.grossMargin.toFixed(1) : '0.0'}%
                            </span>
                          </div>
                        </td>
                      </tr>
