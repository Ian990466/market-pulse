...
export const FairValueGauge = ({ current, low, mid, high }: { current: number, low: number, mid: number, high: number }) => {
  const isValid = typeof current === 'number' && typeof low === 'number' && typeof mid === 'number' && typeof high === 'number' && high > low;
  const percentage = isValid ? Math.min(Math.max(((current - low) / (high - low)) * 100, 0), 100) : 0;
  ...
  if (!isValid) {
    zoneLabel = "No Data";
  } else if (current <= low) { zoneColor = "bg-emerald-500"; zoneLabel = "Strong Buy"; }
    ...
    >
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        <div className="p-3 bg-zinc-950 rounded-lg border border-zinc-800">
          ...
          <p className="text-xl font-bold text-white">{isValid ? formatCurrency(current) : '-'}</p>
        </div>
        <div className="p-3 bg-zinc-950 rounded-lg border border-zinc-800">
          <p className="text-[10px] text-zinc-600 uppercase font-bold">Fair Value (Mid)</p>
          <p className="text-xl font-bold text-white">{isValid ? formatCurrency(mid) : '-'}</p>
        </div>
      </div>
    </motion.div>
  );
